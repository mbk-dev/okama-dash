...
921 tests, three-level pyramid (unit → component → E2E). All tests mock okama —
...
| `poetry run pytest -m component` | Dash callbacks | 572 | ~9s |
| `poetry run pytest -m e2e` | Playwright browser | 42 | ~100s |
| `poetry run pytest -q` | Everything | 921 | ~118s |
| `poetry run pytest -m "not e2e"` | Fast suite | 879 | ~10s |
