...
- Use Poetry with `virtualenvs.in-project = true` — the virtualenv lives at `.venv/` inside the project root.
- When recreating the environment, always use `poetry config virtualenvs.in-project true` before `poetry env use`.
