...
673 tests, three-level pyramid (unit → component → E2E). All tests mock okama —
...
| `poetry run pytest -m component` | Dash callbacks | 429 | ~8s |
| `poetry run pytest -m e2e` | Playwright browser | 25 | ~75s |
| `poetry run pytest -q` | Everything | 673 | ~80s |
| `poetry run pytest -m "not e2e"` | Fast suite | 648 | ~9s |
