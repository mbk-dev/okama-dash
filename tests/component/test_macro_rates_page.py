...
class TestLayoutAndLink:
    ...
    def test_link_callback(self, rates_page):
        link = rates_page.update_rates_link(1, "http://x/macro/rates", ["RUS_CBR.RATE"], "history")
        assert link == "http://x/macro/rates?tickers=RUS_CBR.RATE"

    def test_link_carries_non_default_plot(self, rates_page):
        link = rates_page.update_rates_link(1, "http://x/macro/rates", ["RUS_CBR.RATE"], "real")
        assert "plot=real" in link
