...
class TestHistoryFigure:
    def test_lines_with_country_names(self, cape_page):
        objects = [PicklableIndicator("USA_CAPE10.RATIO"), PicklableIndicator("CHN_CAPE10.RATIO")]
        fig, df = cape_page.get_cape_history_figure(objects)
        assert len(fig.data) == 2
        assert {t.name for t in fig.data} == {"USA", "China"}
        # CAPE is a raw decimal — no ×100 scaling
        assert max(fig.data[0].y) < 100


class TestSnapshot:
    def test_snapshot_collects_all_25_countries_sorted(self, cape_page, patched_indicator):
...
class TestMainCallback:
    def test_history_mode(self, cape_page, patched_indicator):
        fig, config, store = cape_page.update_cape_page(None, ["USA_CAPE10.RATIO", "CHN_CAPE10.RATIO"], "history")
        assert len(fig.data) == 2
        assert isinstance(store, str) and "columns" in store

    def test_snapshot_mode_renders_bar(self, cape_page, patched_indicator):
        with patch.object(cape_page, "get_or_create", side_effect=lambda **kw: (kw["constructor_fn"](), "k")):
            fig, config, store = cape_page.update_cape_page(None, ["USA_CAPE10.RATIO"], "snapshot")
        assert fig.data[0].type == "bar"
        assert len(fig.data[0].y) == 25  # Russia removed
    ...
    def test_snapshot_mobile_keeps_country_labels_outside(self, cape_page, patched_indicator):
        ...
        with patch.object(cape_page, "get_or_create", side_effect=lambda **kw: (kw["constructor_fn"](), "k")):
            fig, config, store = cape_page.update_cape_page(small_screen, ["USA_CAPE10.RATIO"], "snapshot")
    ...
    def test_every_control_is_an_input(self, cape_page):
        ...
        for control in ("cape-series", "cape-plot-type"):
    ...
    def test_error_renders_annotation(self, cape_page):
        with patch.object(cape_page.macro_objects, "get_indicator_object", side_effect=ValueError("nope")):
            fig, config, store = cape_page.update_cape_page(None, ["USA_CAPE10.RATIO"], "history")
        assert fig.layout.annotations[0].text == "nope"
        assert store is None

    def test_empty_selection_prevents_update(self, cape_page):
        with pytest.raises(dash.exceptions.PreventUpdate):
            cape_page.update_cape_page(None, [], "history")
